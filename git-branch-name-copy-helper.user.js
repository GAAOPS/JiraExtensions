...
/* jshint -W097 */
var JExtensions;
(function (JExtensions) {
    'use strict';
    var CopyHelper = /** @class */ (function () {
        function CopyHelper() {
            this.branchIcon = {
        ...
        CopyHelper.prototype.createButtons = function () {
        ...
        CopyHelper.prototype.createIcon = function (icon) {
            var $button = $('<button/>');
        ...
        CopyHelper.prototype.createElement = function (element) {
            return $('<' + element + '/>');
        };
        CopyHelper.prototype.initHeader = function () {
            var _this = this;
            if (this.$header.length) {
                ...
                var clipboard = new Clipboard('.copy-handler', {
                    text: function (trigger) {
                        return _this.$inputClipboard.val();
                    }
                });
            }
        };
        CopyHelper.prototype.setupBranchCopyHandler = function ($button) {
            var _this = this;
            $button.on('click', function () {
                var name = _this.getRawName();
                _this.$inputClipboard.val(name);
            });
        };
        CopyHelper.prototype.setupTimeCopyHandler = function ($button) {
            var _this = this;
            $button.on('click.jira-ext', function () {
                var name = _this.generateBranchName(_this.getRawName());
                _this.$inputClipboard.val(name);
            });
        };
        CopyHelper.prototype.getRawName = function () {
            var $el = this.$header.find(this.projectNameSelector);
            if ($el.length > 1) {
                var jiraIds_1 = "";
                $el.each(function () {
                    jiraIds_1 = jiraIds_1 + $(this).text().split(" ")[0] + "_";
                });
                var workItem = '';
        ...
        CopyHelper.prototype.generateBranchName = function (rawName) {
    ...
    JExtensions.CopyHelper = CopyHelper;
    $(document).ready(function () {
        var copyHelper = new CopyHelper();
        copyHelper.initHeader();
    });
})(JExtensions || (JExtensions = {}));
